...
import com.enation.app.shop.promotion.tool.service.impl.PromotionToolManager;
...
public class OrderPaySendBonusConsumer implements IOrderStatusChangeEvent {
	...
	public void orderChange(OrderStatusChangeMessage orderMessage) {
		if ((orderMessage.getNewStatus().name()).equals(OrderStatus.PAID_OFF.name())) {
			...
			//读取订单的商品信息
			OrderDetail order = this.orderQueryManager.getOneBySn(orderMessage.getOrder().getSn());
			List<OrderSkuVo> list = order.getOrderSkuList();
			for (OrderSkuVo orderSkuVo : list) {
				this.promotionToolManager.reduceQuantity(orderMessage, orderSkuVo);
			}
